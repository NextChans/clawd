...
- **🍚 먹이 주기 & 🖐️ 쓰다듬기** — tray에서 먹이를 주면(고양이가 밥그릇으로 종종걸음쳐
  간다), Grab 모드에서 위에 커서를 올리거나 누르고 있으면 골골거린다.
...
[**Releases**](https://github.com/NextChans/clawd/releases) 페이지에서 최신 **`.dmg`**를
...
릴리스는 `macos-latest` runner에서 GitHub Actions(`.github/workflows/release.yml`)가
빌드한다: **universal DMG**와 **서명된 updater 산출물**(`clawd.app.tar.gz` + `.sig` +
`latest.json`)을 만들고, 자동 생성된 노트와 함께 Release를 게시한다. 트리거 방법은 두
가지다:
