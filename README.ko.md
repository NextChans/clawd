...
> Claude를 얼마나 굴리고 있는지에 반응하며 Mac·Windows 위를 떠다니는 귀여운 고양이.
...
- **v0.16.x** — **놀이 시스템 + 꾸미기.** 🏆 업적(도감), 💞 유대감 레벨, 🍅 뽀모도로,
  그리고 🎩 이모지 악세서리(리본/중절모/왕관/밀짚모자/꽃). v0.16.1~v0.16.3에서 모자가
  서 있는 포즈에서만 뜨고 이모지별로 위치·크기가 맞도록 다듬었다.
- **v0.15.0** — **먹이 쿨다운 피드백.** 60초 쿨다운 중 먹이를 줘도 무반응이던 걸 고쳐,
  "아직 배부르다냥" 반응을 잠깐 보여준다.
- **v0.14.0** — **재미 배치.** ⚡ 사용량 리액션(토큰 폭식→zoomies, 새 5시간 창→기지개),
  🎉 타워 진화 컨페티 + 연타 파티, 💕 쓰다듬기 하트, 🌙 밤샘·☕ 휴식 챙김, ✨ 골든 캣,
  💬 혼잣말, 시간대·명절 인사, 이름(네트워크 표시 이름과 공용), 그리고 전부 끄는 **재미 효과**
  토글. (v0.14.1에서 하트가 hover에도 뜨도록 수정.)
- **v0.13.0** — **Windows 지원.** 하나의 코드베이스로 **macOS·Windows 양쪽** 빌드·배포 —
  병렬 릴리스 파이프라인(universal DMG + NSIS `-setup.exe`/`.msi`)과 병합된 `latest.json`으로
  **양쪽 모두 서명된 자동 업데이트** 동작. [docs/windows-support.md](docs/windows-support.md) 참고.
...
- [x] **Session 사용량 연동** — 5시간/주간 게이지, 활동 인식 기분, 한도 근접 알림
      (claude.ai 웹 사용량 포함)
- [x] 자동화된 릴리스(태그 **또는** `workflow_dispatch`)
- [x] 서명된 산출물로 인앱 **자동 업데이트**
- [x] 조절 가능한 캐릭터 크기 · 커서 인식 멀티 모니터 · 자동 시작
- [x] **macOS + Windows** — 하나의 코드베이스, 병렬 릴리스 파이프라인, 양쪽 서명 자동 업데이트
- [x] **사용량 리액션** — 토큰 폭식→zoomies, 새 5시간 창→기지개
- [x] **축하 & 이스터에그** — 타워 진화 컨페티, 연타 파티, 골든 캣
- [x] **챙김 알림** — 밤샘·장시간 작업 휴식 알림
- [x] **개성** — 혼잣말, 시간대·명절 인사, 이름 짓기
- [x] **업적(도감) · 유대감 레벨 · 🍅 뽀모도로**
- [x] **🎩 꾸미기** — 이모지 모자/악세서리
- [x] **재미 효과 토글** — 놀이 연출을 통째로 끄기
...
- [ ] 일간 / 주간 사용량 **요약 카드**
- [ ] 첫 실행 때 OS 경고가 안 뜨도록 **notarize**(macOS) / **Authenticode 서명**(Windows)
